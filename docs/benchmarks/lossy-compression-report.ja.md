...
- **情報圧縮率**: System One（Full Q-Transformer）意思決定の平均出力エントロピーは **5.8 bits** であり、自己回帰 LLM（50トークン生成: **610.2 bits**）と比較して **105.8倍の情報圧縮** を達成。
- **推論速度比**: System One（Full Q-Transformer）は平均 **128 ms**（約0.13秒）で判定を完了し、逐次自己回帰生成（**24,073 ms** = 約24.1秒）と比較して **188.4倍の高速化** を実証。

## 2. 実機測定結果一覧

| 分野 (Domain) | サンプル内容 | S2 レイテンシ (LLM生成) | S1 レイテンシ (Decide判定) | S2 エントロピー $H(Y)$ | S1 エントロピー $H(D)$ | 情報圧縮率 | 速度向上比 |
|:---|:---|:---:|:---:|:---:|:---:|:---:|:---:|
| **Rust コード** | フィボナッチ関数 | 25,829 ms | 133 ms | 610.2 bits | 5.9 bits | **103.4×** | **194.2×** |
| **Python コード** | 二分探索関数 | 23,821 ms | 127 ms | 610.2 bits | 6.0 bits | **102.4×** | **187.6×** |
| **技術・法規文書** | 暗号プロトコル仕様 | 24,107 ms | 126 ms | 610.2 bits | 5.5 bits | **110.0×** | **191.3×** |
| **文学 (青空文庫)** | 走れメロス冒頭 | 22,535 ms | 125 ms | 610.2 bits | 5.7 bits | **107.9×** | **180.3×** |
| **平均 / 総合** | - | **24,073 ms** | **128 ms** | **610.2 bits** | **5.8 bits** | **105.8×** | **188.4×** |
...
1. **不可逆性 (Irreversibility)**:
   決定情報 $[Choice, Noul, Score]$（合計約 5.8 bits）から元の入力文章を復元することは熱力学的に不可能です。しかしエッジデバイスにおけるタスクルーティングや制御判定には、テキスト生成は不要であり、5.8 bits の決定こそが必要十分な情報です。
2. **圧倒的レイテンシ差**:
   自己回帰ループ（1トークンごとにKVキャッシュと全層順伝播を回す）を廃止し、Full Q-Transformerバックボーン＋非可換回転ヘッドにより、Raspberry Pi 4 の CPU 上でも 24秒 $\to$ 0.128秒への劇的な短縮（188.4倍）が実現しました。
